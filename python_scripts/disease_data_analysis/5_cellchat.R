...
run_single_batch <- function(
    ...
    TOP_N = 5,
    WORKERS = 12,
    OUT_ROOT = file.path(getwd(), "cellchat_by_batch"),
    MIN_CELLS = 0
) {
  ...
  if (ncol(sce_b) < MIN_CELLS) {
    message(sprintf("Batch %s omitido: %d células (< %d).", SAMPLE_ID, ncol(sce_b), MIN_CELLS))
  ...
  tgt_idx_small <- idx_of(c("Mono-S100A8","Macro","Hep","NK","NK-Circ-GNLY","CAF","CAFs","CD4-Treg"))
...
# ========= MAIN: correr todos los batches =========
all_samples <- levels(droplevels(sce$Sample))  # o: unique(as.character(sce$Sample))

message("Se ejecutarán ", length(all_samples), " batches: ",
        paste(all_samples, collapse = ", "))

for (sid in all_samples) {
  message("\n==============================")
  message(">>> Ejecutando batch: ", sid)
  message("==============================")
  try({
    run_single_batch(
      SAMPLE_ID     = sid,
