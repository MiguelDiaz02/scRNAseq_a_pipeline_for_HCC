...
# -*- coding: utf-8 -*-
"""
04_integration/batch_harmonization.py
======================================
Merges the HCC and Healthy unintegrated AnnData objects, harmonizes cell-type
labels, and runs SCVI batch correction.  Outputs a single integrated h5ad with
SCVI latent representation, UMAP coordinates and Leiden clusters.

Configuration
-------------
All I/O paths can be overridden via environment variables:
  HCC_FINAL_PATH     -- path to HCC unintegrated h5ad
  HEALTHY_FINAL_PATH -- path to Healthy unintegrated h5ad
  OUT_DIR            -- output directory (figures/ sub-dir created automatically)
  OUT_H5AD           -- path for the integrated output h5ad
"""
...
# ----------------------------
HCC_FINAL_PATH = os.getenv(
    "HCC_FINAL_PATH",
    "/home/mdiaz/HCC_project/hcc_adata/1_5T_unintegrated.h5ad",
)
HEALTHY_FINAL_PATH = os.getenv(
    "HEALTHY_FINAL_PATH",
    "/home/mdiaz/HCC_project/healthy_adata/1_5C_unintegrated.h5ad",
)
OUT_DIR = os.getenv(
    "OUT_DIR",
    "/home/mdiaz/HCC_project/integration",
)
OUT_FIG_DIR = os.path.join(OUT_DIR, "figures")
OUT_H5AD = os.getenv(
    "OUT_H5AD",
    os.path.join(OUT_DIR, "scvi_integrated.h5ad"),
)
...
def standardize_varnames(ad: sc.AnnData) -> sc.AnnData:
    """Drop trailing '.digits', strip whitespace, enforce uniqueness."""
...
def intersect_genes(a: sc.AnnData, b: sc.AnnData) -> Tuple[sc.AnnData, sc.AnnData]:
    """Return copies of a and b restricted to their common genes."""
...
def _singularize_and_title(s: str) -> str:
    """Apply plural fix or title-case the string."""
    low = s.lower().strip()
...
def _is_gene_token(tok: str) -> bool:
    """Crude heuristic: uppercase letters/numbers, length 2-8 (e.g., GZMK, VWF)."""
    return bool(re.fullmatch(r"[A-Z0-9]{2,8}", tok))

def _mk_parenthetic(gene: Optional[str]) -> str:
    """Format a gene name as a parenthetical suffix."""
    return f" ({gene}+)" if gene else ""

def harmonize_label_preserving_granularity(raw: str) -> str:
    """
    Convert many label variants to short, intuitive but still specific labels.
...
def pick_label_column(ad: sc.AnnData, candidates: Optional[List[str]] = None) -> Optional[str]:
    """Return the first obs column name from candidates that exists in ad."""
...
def _compute_umap(adata: sc.AnnData, use_rep: Optional[str] = None) -> None:
    """Compute neighbors and UMAP; optionally use a precomputed representation."""
    if use_rep is None:
        sc.pp.pca(adata)
...
def _fancy_umap(
    ...
    """
    Render a styled UMAP scatter with per-cluster median labels.

    Parameters
    ----------
    adata      : AnnData with X_umap in obsm.
    label      : obs column to color by.
    ...
    ax.scatter(x, y, s=point_size, c=colors, linewidths=0, alpha=0.9)
...
# ----------------------------
def main():
    """Run the full batch harmonization and SCVI integration pipeline."""
