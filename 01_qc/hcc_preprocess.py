...
"""
HCC-only preprocessing and QC pipeline for single-cell RNA-seq data.

What this script does:
- Loads HCC AnnData files into a list (`adatas`) and standardizes metadata.
...
- Harmonizes DX labels for tumor/adjacent tissues in HCC samples.
...
INPUT_DIR  = os.getenv('INPUT_DIR',  '/home/mdiaz/sc_liver_data/hcc_h5ad_adata')
OUTPUT_DIR = os.getenv('OUTPUT_DIR', '/home/mdiaz/HCC_project/hcc_adata')
OUTPUT_H5AD = os.getenv('OUTPUT_H5AD', os.path.join(OUTPUT_DIR, '0C_doub_remov.h5ad'))
...
# Load HCC samples
# ----------------------------------------------------------------------
adatas = [x for x in os.listdir(INPUT_DIR) if x.endswith('.h5ad')]

def load_it(adata):
    """Load one HCC AnnData file and standardize metadata fields."""
    samp = adata.split('_')[0]
    dx = adata.split('_')[1]
    adata = sc.read_h5ad(os.path.join(INPUT_DIR, adata))
...
def mad_outlier(adata, metric, nmads, upper_only=False):
    """Return boolean mask of MAD-based outliers for the given metric."""
    M = adata.obs[metric]
    if not upper_only:
...
def pp(adata):
    """Apply MT content filter and MAD-based outlier removal."""
...
# ----------------------------------------------------------------------
for adata in adatas:
    adata.obs['DX'] = adata.obs['DX'].replace(['tumor', '4T', '5T'], 'tumor tissue')
    adata.obs['DX'] = adata.obs['DX'].replace(['1NT', '2NT', '3NT', '4NT', '5NT'], 'adjacent tissue')

df['DX'] = df['DX'].replace(['tumor', '4T', '5T'], 'tumor tissue')
df['DX'] = df['DX'].replace(['1NT', '2NT', '3NT', '4NT', '5NT'], 'adjacent tissue')
...
print(adata)
adata.layers["counts"] = adata.X.copy() if sp.issparse(adata.X) else np.array(adata.X, copy=True)
adata.raw = adata.copy()
