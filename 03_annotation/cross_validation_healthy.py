...
Config variables (override via environment):
  REF3_DIR      — directory containing the healthy reference .h5ad
  REF3_FILE     — filename of the healthy reference .h5ad
...
OUT_BASE  = os.getenv("HEALTHY_CV_OUTDIR", "/home/mdiaz/HCC_project/Healthy_validation_analysis")
...
def canonicalize_celltypes(
    ...
    """Map raw cell_type strings to canonical liver cell type names."""
    mapping = {
...
def stratified_kfold_indices(labels: pd.Series, n_splits: int = N_SPLITS, random_state: int = SEED) -> List[Tuple[np.ndarray, np.ndarray]]:
    """Return list of (train_idx, test_idx) arrays from stratified k-fold."""
...
def sanitize_labels_for_scanvi(adata: sc.AnnData, key: str = "CellType", unlabeled: str = "Unknown") -> None:
    """Fill NaN/empty/variant-null labels with unlabeled and build a clean Categorical."""
    s = adata.obs[key]
    ...
    clean_cats = [c for c in cat.categories if (c is not None) and (c == c)]
...
def run_fold(
    ...
    """Train SCVI/SCANVI on one fold, predict on test set, and save all artifacts."""
    ...
    ad_test = ad_test[:, ad_train.var_names.intersection(ad_test.var_names)].copy()
    ...
    scanvi_test.train(max_epochs=0)
    ...
    sc.pl.umap(
        ad_comb, color="CellType_true", ax=ax[0], show=False,
        title="TEST: true CellType", legend_loc="right margin",
        legend_fontsize=7, size=3, frameon=False, use_raw=False,
    )
    if "predicted" in ad_comb.obs.columns:
        sc.pl.umap(
            ad_comb, color="predicted", ax=ax[1], show=False,
            title="TEST: predicted (SCANVI)", legend_loc="right margin",
            legend_fontsize=7, size=3, frameon=False, use_raw=False,
        )
    for axi in ax:
