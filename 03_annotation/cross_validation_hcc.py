...
Usage (CLI):
  python cross_validation_hcc.py \
      --adata_path /home/mdiaz/HCC_project/hcc_adata/1_5T_unintegrated.h5ad \
      --outdir /home/mdiaz/HCC_project/HCC_analysis/Diseased_CV \
      --canon_map /home/mdiaz/manuscript_revision/manuscript_code/scRNAseq_a_pipeline_for_HCC/config/canonical_map_hcc.json

Config variables (override via environment or CLI flags):
  ADATA_PATH        — path to diseased AnnData .h5ad
  CV_OUTDIR         — output directory base
  CANON_MAP_PATH    — path to canonical map JSON
"""
...
# ----------------------------
# Config variables (override via environment; CLI flags take precedence)
# ----------------------------
ADATA_PATH     = os.getenv("ADATA_PATH", "/home/mdiaz/HCC_project/hcc_adata/1_5T_unintegrated.h5ad")
CV_OUTDIR      = os.getenv("CV_OUTDIR", "/home/mdiaz/HCC_project/HCC_analysis/Diseased_CV")
CANON_MAP_PATH = os.getenv(
    "CANON_MAP_PATH",
    os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        "config", "canonical_map_hcc.json"
    )
)
...
def standardize_varnames(adata: AnnData) -> None:
    """Drop trailing .digits suffixes from var_names and enforce uniqueness."""
...
def load_canonical_map(path_json: Optional[str]) -> Optional[Dict[str, str]]:
    """Load a JSON dict mapping raw label strings to canonical names."""
    if not path_json:
        return None
...
def apply_canonical_map(
    ...
    """Apply case-insensitive canonical map to adata.obs[label_col] in place."""
...
def sanitize_labels_for_scanvi(adata: AnnData, label_col: str, unknown_label: str = "Unknown") -> None:
    """Fill NaN/empty labels with unknown_label and build a clean Categorical."""
    if label_col not in adata.obs.columns:
...
def filter_classes_by_support(
    ...
    """Return (kept, rare) label lists based on cell count and group coverage."""
...
def align_by_gene_intersection(ad_train: AnnData, ad_test: AnnData) -> Tuple[AnnData, AnnData]:
    """Subset both AnnData objects to their shared var_names."""
...
def compute_unseen_and_oott(
    ...
    """Recode test labels absent from training set to unknown_label."""
...
def safe_stratified_group_kfold(labels: pd.Series, groups: pd.Series, n_splits: int, seed: int):
    """Return list of (train_idx, test_idx) arrays from stratified group k-fold."""
...
def plot_pred_score_hist(pred_scores: np.ndarray, out_png: str) -> None:
    """Save histogram of prediction scores."""
...
def plot_umap_true_vs_pred(ad_test: AnnData, label_col: str, pred_col: str, out_png: str) -> None:
    """Compute UMAP on test set and save side-by-side true vs. predicted plots."""
...
def plot_per_class_metrics(df_report: pd.DataFrame, out_png: str) -> None:
    """Bar plot of per-class precision/recall/F1."""
    df = df_report.copy()
...
def plot_summary_metrics(df_report: pd.DataFrame, out_png: str) -> None:
    """Bar plot of accuracy/macro/weighted summary metrics."""
...
def compute_and_plot_pr_curves(y_true: pd.Series, y_proba_df: pd.DataFrame, classes_for_metrics: List[str], outdir: str) -> pd.DataFrame:
    """Compute precision-recall curves per class and save PNGs + average_precision CSV."""
...
def dataframe_to_confusion_and_counts(y_true: pd.Series, y_pred: pd.Series, label_order: List[str]):
    """Return (confusion_matrix_df, true_counts_df, pred_counts_df)."""
...
def train_scvi_scanvi_on_train(
    ...
    """Train SCVI then SCANVI on ad_train using the specified counts layer."""
    set_all_seeds(seed)
...
def map_and_predict_on_test(
    ...
    """Transfer scanvi_model predictions to ad_test; return (y_pred_cls, pred_score, y_proba_df, ad_test)."""
    set_all_seeds(seed)
    ...
    try:
        qmodel = SCANVI.load_query_data(ad_test, scanvi_model)
...
def run_fold(fold_id: int, adata: AnnData, train_idx, test_idx, cfg, outdir_fold: str) -> None:
    """Run a single CV fold: train SCVI/SCANVI, predict on test, save all artifacts."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--adata_path", default=ADATA_PATH, type=str, help="Path to diseased AnnData .h5ad")
    p.add_argument("--outdir", default=CV_OUTDIR, type=str, help="Output directory base")
    ...
    p.add_argument("--canon_map", default=CANON_MAP_PATH, type=str, help="Path to canonical map JSON")
