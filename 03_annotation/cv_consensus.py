...
"""
cv_consensus.py
Consensus CV summarizer (supports *_inclUnknown/*_exclUnknown variants).

Consolidates fold-level artifacts produced by cross_validation_hcc.py into a
single consensus directory with aggregated metrics, confusion matrices, and
merge suggestions.

Usage:
  python cv_consensus.py \
      --cv_outdir /home/mdiaz/HCC_project/HCC_analysis/Diseased_CV \
      --export_prefix DiseasedCV_ \
      --unknown_policy auto \
      --collect_pred_scores true

Config variables (override via environment or CLI):
  CV_OUTDIR         — directory containing fold*_DiseasedCV/ subdirectories
  EXPORT_PREFIX     — prefix for output file names (default 'DiseasedCV_')

unknown_policy: auto | excl | incl
  - auto: tries canonical name, then *_exclUnknown, then *_inclUnknown
  - excl: prioritizes *_exclUnknown
  - incl: prioritizes *_inclUnknown
"""
...
# ----------------------------
CV_OUTDIR     = os.getenv("CV_OUTDIR",     "/home/mdiaz/HCC_project/HCC_analysis/Diseased_CV")
EXPORT_PREFIX = os.getenv("EXPORT_PREFIX", "DiseasedCV_")


def find_variant_file(fold_dir: str, base_name: str, policy: str, variants: list) -> Optional[str]:
    """
    Resolve files that may come in 'inclUnknown'/'exclUnknown' variants.

    base_name: canonical base filename (e.g., 'classification_report.csv')
    variants: list of (suffix, alias) -> [('_exclUnknown.csv','excl'), ('_inclUnknown.csv','incl')]
    policy: 'auto' | 'excl' | 'incl'
    Returns path string or None if not found.
    """
    exact = os.path.join(fold_dir, base_name)
    if os.path.exists(exact):
        return exact

    if policy == "excl":
        ordered = [v for v in variants if v[1] == "excl"] + [v for v in variants if v[1] != "excl"]
    elif policy == "incl":
        ordered = [v for v in variants if v[1] == "incl"] + [v for v in variants if v[1] != "incl"]
    else:
        ordered = variants

    for suf, _alias in ordered:
        cand = os.path.join(fold_dir, base_name.replace(".csv", suf))
...
def parse_args():
    p = argparse.ArgumentParser(description="Build consensus summary from CV folds.")
    p.add_argument("--cv_outdir", default=CV_OUTDIR, help="Directory containing fold*_DiseasedCV/ etc.")
    p.add_argument("--export_prefix", default=EXPORT_PREFIX, help="Prefix for output file names.")
    p.add_argument("--unknown_label", default="Unknown", help="Name of unknown/unmapped label.")
    ...
    p.add_argument("--unknown_policy", default="auto", choices=["auto", "excl", "incl"],
                   help="Prefer *_exclUnknown or *_inclUnknown when both exist (default auto).")
    args = p.parse_args()
...
def setup_logging():
    """Configure root logger to INFO level with a simple format."""
    logging.basicConfig(level=logging.INFO, format="[%(levelname)s] %(message)s")


def ensure_outdir(base_dir: str) -> str:
    """Create and return the consensus output subdirectory."""
    outdir = os.path.join(base_dir, "consensus")
    os.makedirs(outdir, exist_ok=True)
    return outdir


def discover_folds(cv_outdir: str) -> List[str]:
    """Return sorted list of fold directories matching fold*_DiseasedCV pattern."""
    candidates = sorted(glob(os.path.join(cv_outdir, "fold*_DiseasedCV")))
    if not candidates:
        logging.error("No fold directories found under %s", cv_outdir)
        sys.exit(1)
...
def read_csv_safe(path: str, **kwargs) -> Optional[pd.DataFrame]:
    """Read a CSV file, returning None and logging a warning on failure."""
...
def read_classification_report(fold_dir: str, policy: str = "auto") -> Optional[pd.DataFrame]:
    """Load and normalize classification_report.csv from a fold directory."""
    path = find_variant_file(
        fold_dir, "classification_report.csv", policy,
        [("_exclUnknown.csv", "excl"), ("_inclUnknown.csv", "incl")]
    )
    if path is None:
...
def extract_global_metrics(rep: pd.DataFrame, fold_id: str) -> pd.DataFrame:
    """Extract accuracy, macro, and weighted metrics from a classification report DataFrame."""
    rows = []
    if rep is None:
        return pd.DataFrame()
    rep = rep.copy()
...
def extract_per_class(rep: pd.DataFrame, fold_id: str) -> pd.DataFrame:
    """Extract per-class rows from a classification report DataFrame."""
    if rep is None:
        return pd.DataFrame()
    rep = rep.copy()
    rep.index = rep.index.astype(str)
...
def read_ap_by_class(fold_dir: str, fold_id: str, policy: str = "auto") -> pd.DataFrame:
    """Load average_precision_by_class.csv from a fold directory."""
    path = find_variant_file(
        fold_dir, "average_precision_by_class.csv", policy,
        [("_exclU.csv", "excl"), ("_inclU.csv", "incl")]
    )
    if path is None:
...
def read_oott(fold_dir: str, fold_id: str) -> pd.DataFrame:
    """Load oott_rate.csv from a fold directory."""
...
def read_confusion(fold_dir: str) -> Optional[pd.DataFrame]:
    """Load confusion_table.csv from a fold directory."""
...
def summarize_numeric(df: pd.DataFrame, value_cols: List[str], by: List[str]) -> pd.DataFrame:
    """Aggregate value_cols by grouping keys, computing mean/std/median."""
...
def label_stability(row, f1_mean_cut=0.80, f1_sd_cut=0.05, support_total_cut=0):
    """Classify a per-class summary row as 'stable', 'intermediate', or 'problematic'."""
...
def row_normalize(cm: pd.DataFrame) -> pd.DataFrame:
    """Row-normalize a confusion matrix DataFrame."""
    cm = cm.copy()
...
def top_confusions(cm_row: pd.DataFrame, top_k: int = 20) -> pd.DataFrame:
    """Return the top-k off-diagonal entries of a row-normalized confusion matrix."""
    records = []
...
def symmetric_confusion(cm_row: pd.DataFrame) -> pd.DataFrame:
    """Return symmetric pairwise confusion scores (mean of both directions)."""
...
def propose_merges(sym_df: pd.DataFrame, per_class_cons: pd.DataFrame, ap_cons: pd.DataFrame,
    ...
    """Suggest class pairs that might be merged based on symmetric confusion + low F1/AP."""
    if sym_df is None or sym_df.empty:
        return pd.DataFrame(columns=["class_A", "class_B", "sym_conf", "F1_A", "F1_B", "AP_A", "AP_B", "support_A", "support_B"])
    f1map = per_class_cons.set_index("class")["f1_mean"].to_dict()
    supmap = per_class_cons.set_index("class")["support_total"].to_dict()
    apmap = ap_cons.set_index("class")["AP_mean"].to_dict() if not ap_cons.empty else {}
    rows = []
    ...
    return pd.DataFrame(rows).sort_values("sym_conf", ascending=False)


def plot_per_class_bar(df: pd.DataFrame, value_col: str, out_png: str, dpi: int = 300):
    """Save horizontal bar plot of per-class metric values."""
    if df.empty:
        return
...
def plot_confusion_heatmap(cm_row: pd.DataFrame, out_png: str, dpi: int = 300):
    """Save heatmap of row-normalized confusion matrix (diagonal masked)."""
    if cm_row.empty:
        return
...
def pred_score_hist(df_ps_all: pd.DataFrame, out_png: str, dpi: int = 300):
    """Save histogram of per-cell prediction scores across all folds."""
    if df_ps_all.empty:
        return
    plt.figure()
...
def pred_score_violin(df_ps_all: pd.DataFrame, out_png: str, dpi: int = 300, max_classes: int = 40):
    """Save violin plot of per-cell prediction scores grouped by predicted class."""
    if df_ps_all.empty:
        return
...
def read_pred_scores_from_h5ad(fold_dir: str, label_pred: str = "CellType_pred") -> pd.DataFrame:
    """Load per-cell pred_score and predicted label from test_predicted.h5ad."""
    if ad is None:
...
def main():
    ...
    for fold_dir in folds:
        ...
        rep = read_classification_report(fold_dir, policy=args.unknown_policy)
        if rep is not None:
            globals_tbl.append(extract_global_metrics(rep, fold_id))
            per_class_tbl.append(extract_per_class(rep, fold_id))

        ap_tbl.append(read_ap_by_class(fold_dir, fold_id, policy=args.unknown_policy))
        ...
        if cm is not None and not cm.empty:
            all_labels.update(cm.index.astype(str).tolist())
            all_labels.update(cm.columns.astype(str).tolist())
            cm = cm.copy()
            ...
            if cm_sum is None:
                ...
                cm_sum = cm_sum.reindex(index=sorted(set(cm_sum.index).union(cm.index)),
                                        columns=sorted(set(cm_sum.columns).union(cm.columns)),
                                        fill_value=0)
                cm = cm.reindex(index=cm_sum.index, columns=cm_sum.columns, fill_value=0)
                cm_sum = cm_sum.add(cm, fill_value=0)

        if args.collect_pred_scores:
            ps = read_pred_scores_from_h5ad(fold_dir, label_pred="CellType_pred")
            if not ps.empty:
                ps["fold"] = fold_id
    ...
    if ap_tbl:
        non_empty = [x for x in ap_tbl if x is not None and not x.empty]
        if non_empty:
            df_ap_all = pd.concat(non_empty, ignore_index=True)
    ...
    if oott_tbl:
        non_empty = [x for x in oott_tbl if x is not None and not x.empty]
        if non_empty:
            oott_all = pd.concat(non_empty, ignore_index=True)
